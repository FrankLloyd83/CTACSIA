...
data=pd.read_csv('DN_Full_Voxels.csv', sep = ';')
...
def update_metrics():
    ...
    if reset:
        train_loss = memory.history['loss']
        val_loss = memory.history['val_loss']
        train_acc = memory.history['accuracy']
        val_acc = memory.history['val_accuracy']
    else:
        train_loss = np.concatenate((train_loss, 
                                     np.array(memory.history['loss'])), axis=0)
        val_loss = np.concatenate((val_loss, 
                                   np.array(memory.history['val_loss'])), axis=0)
        train_acc = np.concatenate((train_acc, 
                                    np.array(memory.history['accuracy'])), axis=0)
        val_acc = np.concatenate((val_acc, 
                                  np.array(memory.history['val_accuracy'])), axis=0)
